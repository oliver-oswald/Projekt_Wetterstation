...
IPAddress local_IP(192, 168, 178, 242);
IPAddress gateway(192, 168, 178, 1);
IPAddress subnet(255, 255, 255, 0);
unsigned int localUdpPort = 4212;           //local port to listen on
...
void loop() {
  ...
  if (currentMillis - previousMillis >= intervalle){
     ...
     if (isnan(humidity)||isnan(temp_f)){
      ...
      MeasurementString = "Val3 " + String(temp_f, 1) + " " + String(int(humidity)) + "EE";  //'E' is the terminator for the string.
